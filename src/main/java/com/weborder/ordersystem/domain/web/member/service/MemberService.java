package com.weborder.ordersystem.domain.web.member.service;
...
import com.weborder.ordersystem.domain.erp.service.CustomerService;
import com.weborder.ordersystem.domain.web.member.dto.MemberCreateRequest;
import com.weborder.ordersystem.domain.web.member.dto.MemberResponse;
import com.weborder.ordersystem.domain.web.member.dto.MemberUpdateRequest;
import com.weborder.ordersystem.domain.web.member.dto.PasswordChangeRequest;
...
import com.weborder.ordersystem.global.auth.dto.FindMemberIdResponse;
...
public class MemberService {
    ...
    public Page<MemberResponse> getMembers(Pageable pageable) {
        return memberRepository.findMembersWithConditions(
    ...
    public Page<MemberResponse> searchMembers(String memberId, String memberName, String custCodeName,
                                             MemberRole role, Boolean useYn, Integer hbsb, Pageable pageable) {
        ...
        if (custCodeName == null || custCodeName.trim().isEmpty()) {
            Page<Member> memberPage = memberRepository.findMembersWithConditions(
                memberId, memberName, null, role, useYn, hbsb, pageable);
        ...
        Page<Member> memberPage = memberRepository.findMembersWithCustCodes(
            memberId, memberName, matchingCustCodes, role, useYn, hbsb, pageable);
        
        return memberPage.map(this::convertToResponseWithCustName);
    }

    /**
     * 통합 키워드 검색 (회원ID, 회원명, 거래처명을 OR로 검색) + 필터
     */
    public Page<MemberResponse> searchByKeyword(String keyword, MemberRole role, Boolean useYn, Integer hbsb, Pageable pageable) {
        if (keyword == null || keyword.trim().isEmpty()) {
            Page<Member> memberPage = memberRepository.findMembersWithConditions(
                null, null, null, role, useYn, hbsb, pageable);
            return memberPage.map(this::convertToResponseWithCustName);
        }
        ...
        List<Integer> matchingCustCodes = customerRepository.findAll().stream()
                .filter(c -> c.getCustCodeName() != null &&
                       c.getCustCodeName().toLowerCase().contains(kw.toLowerCase()))
                .map(c -> c.getCustCodeCode())
                .collect(Collectors.toList());

        Page<Member> memberPage;
        if (matchingCustCodes.isEmpty()) {
            memberPage = memberRepository.findByKeyword(kw, role, useYn, hbsb, pageable);
        } else {
            memberPage = memberRepository.findByKeywordOrCustCodes(kw, matchingCustCodes, role, useYn, hbsb, pageable);
        }
    ...
    public MemberResponse updateMember(Long memberId, MemberUpdateRequest request) {
        ...
        if (request.getHbsb() != null) {
            member.updateHbsb(request.getHbsb(), request.getUpdateBy());
        }
    ...
    public void deleteMember(Long memberId, String loginId) {
        ...
        member.updateUseYn(false, loginId);
        memberRepository.save(member);
        log.info("회원 소프트삭제 완료 (비활성화) - ID: {}", member.getMemberId());
    }
