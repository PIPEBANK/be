...
public interface StockDateRepository extends JpaRepository<StockDate, StockDate.StockDateId> {
    ...
    // 전체 재고 목록 (부서7 기준, 품목정보 포함 검색)
    @Query("SELECT s FROM StockDate s WHERE s.stockDateBuse = 7 AND s.stockDateCnt != 0 ORDER BY s.stockDateItem")
    List<StockDate> findAllStockBuse7();

    // 특정 소속의 재고 목록
    @Query("SELECT s FROM StockDate s WHERE s.stockDateSosok = :sosok AND s.stockDateBuse = 7 ORDER BY s.stockDateItem")
    List<StockDate> findStockBySosokAndBuse7(@Param("sosok") Integer sosok);
