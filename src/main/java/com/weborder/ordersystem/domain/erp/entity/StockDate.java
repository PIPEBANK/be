...
public class StockDate {
    ...
    @Id
    @Column(name = "STOCK_DATE_ITEM", columnDefinition = "int(11)", nullable = false)
    private Integer stockDateItem; // 품목코드

    @Id
    @Column(name = "STOCK_DATE_SOSOK", columnDefinition = "int(11)", nullable = false)
    private Integer stockDateSosok; // 소속

    @Id
    @Column(name = "STOCK_DATE_BUSE", columnDefinition = "int(11)", nullable = false)
    ...
    // 재고 수정
    public void updateStock(BigDecimal cnt, String luser) {
        this.stockDateCnt = cnt;
        this.stockDateLdate = LocalDateTime.now();
        this.stockDateLuser = luser;
    }

    // 재고 증감
    public void adjustStock(BigDecimal adjustment, String luser) {
        this.stockDateCnt = this.stockDateCnt.add(adjustment);
        this.stockDateLdate = LocalDateTime.now();
        this.stockDateLuser = luser;
    }
